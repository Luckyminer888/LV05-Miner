...
{
    xTaskCreate(&restart, "restartTask", 4096, NULL, 5, &restartTask); // 创建重启任务
    const char resp[] = "{ \"code\": 0 }";
    httpd_resp_send(req, resp, strlen(resp));
    return ESP_OK;
}
...
static esp_err_t GET_system_info(httpd_req_t * req)
{
    ...
    cJSON_AddStringToObject(root, "initMiner", "lucky");
